...
describe(`[${TabWriterService.name}]`, () => {
  ...
  describe('[writeParsedInstructionToTab]', () => {
    const buildParsedDefaultInstructionResult = (chord: number, note: string): ParserResult => {
      const instruction = `${chord}-${note}`;
      return new ParserResult(instruction, 0, instruction.length - 1, null, null, null);
    };
