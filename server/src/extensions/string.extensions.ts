...
String.prototype.indexOfDifferent = function (this: string, char: string, start = 0, iteration = 1): number {
  ...
  const positiveIterationEndVerification: (idx: number) => boolean = (idx) => idx < this.length;
  const negativeIterationEndVerification: (idx: number) => boolean = (idx) => idx > 0;
  const hasEnded = iteration > 0 ? positiveIterationEndVerification : negativeIterationEndVerification;
