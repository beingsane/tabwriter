...
describe(`[${Parser.name}]`, () => {
  describe(`[parse]`, () => {
    ...
    it('should read instructions with brackets as a single instruction, even with spaces between', () => {
      const instructionsStr =
        '  instr1  (  arg1  , arg2  )  [  arg1  , arg2  ]  {  arg1  , arg2  }  ';
      const parser = new Parser();
    ...
    it('should parse and read the instruction targets', () => {
      ...
      const instructions = parser.parse(instructionsStr);
      const readTargetsValues = instructions[0].targets?.map(
        (target) => target.value
      );
    ...
    it('should upadete inner targets positions to the global reference', () => {
      ...
      const innerTargetsRead = instructionReadTarget
        ? instructionReadTarget[0]?.targets
        : null;
      ...
      if (innerTargetsRead) {
        innerTargets.forEach((target, idx) => {
          ...
          expect(innerTargetsRead[idx].readFromIdx).toBe(
            instructionsStr.indexOf(target)
          );
          expect(innerTargetsRead[idx].readToIdx).toBe(
            instructionsStr.indexOf(target) + target.length - 1
          );
        });
      }
    });
