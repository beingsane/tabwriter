...
import {
  TabWriterService,
  TabWriterInstructions,
} from '../../services/tabWriter.service';
...
import { InvalidInstructionError } from './models/invalidInstructionError.model';
...
export class TabController extends BaseController {
  ...
  constructor(childControllers?: BaseController[]) {
    ...
    this.router.post(
      '/',
      validateInputs(checkSchema(tabCreationSchema)),
      asyncHandler(this.createTab)
    );
  }

  public async createTab(req: Request, res: Response): Promise<void> {
    const tabInstructions = req.body as TabWriterInstructions;
    const tabWriterBuildResult = await TabWriterService.writeTab(
      tabInstructions
    );
    ...
    if (tabWriterBuildResult.success) {
      ...
      const failedInstructionsResponseErrors = tabWriterBuildResult.instructionsResults
        ...
        .map(
          (failedInstructionResult) =>
            new InvalidInstructionError(failedInstructionResult)
        );

      const response = new ResponseErrorInvalidInstructions(
        failedInstructionsResponseErrors
      );
