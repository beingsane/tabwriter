...
describe(`[${InvalidInstructionError.name}]`, () => {
  it('should be mapped to the unprocessable instruction error code', () => {
    const responseError = new InvalidInstructionError(
      getDefaultInvalidInstructionWriteResult()
    );
  ...
  it('should set the instruction to the write result instruction', () => {
    ...
    expect(responseError.instruction).toBe(
      invalidInstructionWriteResult.instruction
    );
  });
  ...
  it('should set the readFrom to the write result readFromIdx', () => {
    ...
    expect(responseError.readFrom).toBe(
      invalidInstructionWriteResult.readFromIdx
    );
  });
