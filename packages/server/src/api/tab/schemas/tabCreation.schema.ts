...
export const tabCreationSchema: Record<string, ParamSchema> = {
  instructions: {
    exists: {
      errorMessage: (): InvalidInputError =>
        new MissingParameter('instructions'),
    },
    isString: {
      errorMessage: (): InvalidInputError =>
        new InvalidType('instructions', 'string'),
    },
    trim: true,
    notEmpty: {
  ...
  rowsQuantity: {
    exists: {
      errorMessage: (): InvalidInputError =>
        new MissingParameter('rowsQuantity'),
    },
    isInt: {
      options: {
      ...
      errorMessage: (value: string): InvalidInputError => {
        return Number.isInteger(Number(value))
          ? new InvalidIntRange('rowsQuantity', {
              min: MIN_ROWS_QUANTITY,
              max: MAX_ROWS_QUANTITY,
            })
  ...
  rowsSpacing: {
    exists: {
      errorMessage: (): InvalidInputError =>
        new MissingParameter('rowsSpacing'),
    },
    isInt: {
      options: {
