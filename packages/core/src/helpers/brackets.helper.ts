export class BracketsHelper {
  ...
  public static openingBrackets: string[] = Object.keys(
    BracketsHelper.openingClosingBracketsMap
  );
  ...
  public static indexOfMatchingClosingBracket(
    str: string,
    ...
    if (!BracketsHelper.isOpeningBracket(str[openingBracketIndex]))
      throw Error(
        `Invalid opening bracket ${str[openingBracketIndex]}, at index ${openingBracketIndex}`
      );

    const openingBracket = str[openingBracketIndex];
    const closingBracket =
      BracketsHelper.openingClosingBracketsMap[openingBracket];
  ...
  public static getValueInsideBrackets(
    str: string,
    openingBracket: string,
    start?: number
  ): string {
    if (!BracketsHelper.isOpeningBracket(openingBracket))
      throw Error(`Invalid openingBracket ${openingBracket}`);

    const startIdx = str.indexOf(openingBracket, start);
    if (startIdx > -1) {
      const endIdx = BracketsHelper.indexOfMatchingClosingBracket(
        str,
        startIdx
      );
      return endIdx < 0
        ? str.slice(startIdx + 1, str.length)
        : str.slice(startIdx + 1, endIdx);
    }
  ...
  private static hasBrackets(str: string, brackets: string[]): boolean {
    const bracketsIdx = brackets
      .map((bracket) => str.indexOf(bracket))
      .filter((bracketIdx) => bracketIdx > -1);
    const firstBracketIdx =
      bracketsIdx.length > 0 ? Math.min(...bracketsIdx) : -1;
