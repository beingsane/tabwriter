import { InvalidArgumentError } from '../common/invalidArgument.error';
import { Tab } from './tab';
import { TabBlockWriteInstruction } from './tabBlockWriteInstruction';
import { TabBlockWriteResult } from './tabBlockWriteResult';
...
export class TabBlock {
  ...
  public addSpacing(spacing?: number): void {
    if (spacing !== undefined && spacing < 1)
      throw new InvalidArgumentError('spacing must be a positive number');
    ...
    this.internalRows.forEach(
      (row, rowIdx) => (this.internalRows[rowIdx] = row + rowFiller)
    );
  ...
  public getMaximumRemovableRowsSpacing(): number {
    return this.internalRows.reduce((store: number, row) => {
      const nonFillerLastIdx = row.indexOfDifferent(
        this.tab.rowsFiller,
        row.length - 1,
        -1
      );
  ...
  public writeInstruction(
    instruction: TabBlockWriteInstruction
  ): TabBlockWriteResult {
    return this.writeInstructionsMerged([instruction]);
  }

  public writeInstructionsMerged(
    instructions: TabBlockWriteInstruction[]
  ): TabBlockWriteResult {
    const invalidChords = this.getInvalidChordsToWrite(instructions);
    if (invalidChords.length > 0)
      return new TabBlockWriteResult(
        false,
        this.getInvalidChordsToWriteDescription(invalidChords)
      );

    const duplicatedChords = this.getChordsWithMultipleInstructions(
      instructions
    );
    if (duplicatedChords.length > 0)
      return new TabBlockWriteResult(
        false,
        this.getChordsWithMultipleInstructionsDescription(duplicatedChords)
      );
  ...
  public writeHeader(header: string): TabBlockWriteResult {
    if (header.trim().length === 0)
      return new TabBlockWriteResult(
        false,
        'Nenhum conteúdo indicado para escrita.'
      );

    this.setupForNewSection();
    const headerToAdd =
      this.tab.sectionSymbol + this.tab.sectionFiller + header;
    this.internalHeader +=
      headerToAdd + this.getSectionsFiller(this.tab.rowsSpacing);

    this.internalRows.forEach(
      (row, idx) => (this.internalRows[idx] = row + this.tab.sectionSymbol)
    );
    this.addSpacing();
  ...
  public writeFooter(footer: string): TabBlockWriteResult {
    ...
    const {
      ...
      fillersToAdd,
    } = this.getFooterInsertPreparation(footer);

    if (fillersToAdd > 0) {
      this.internalHeader += this.getSectionsFiller(fillersToAdd);
      this.internalRows.forEach(
        (row, idx) =>
          (this.internalRows[idx] = row + this.getRowFiller(fillersToAdd))
      );
    }

    const sectionFinalizer =
      this.tab.sectionSymbol + this.getSectionsFiller(this.tab.rowsSpacing);
    this.internalFooter =
      this.internalFooter.slice(0, footerInsertStartIdx) +
      footerToAdd +
      sectionFinalizer;
  ...
  private getFooterInsertPreparation(footer: string): FooterInsertPreparation {
    const footerToAdd =
      this.getSectionsFiller(this.tab.rowsSpacing) +
      footer +
      this.tab.sectionFiller;
    const nonSectionFillerFooterIdx = this.internalFooter.indexOfDifferent(
      this.tab.sectionFiller,
      -1,
      -1
    );
    ...
    if (footerInsertStartIdx + footerToAdd.length <= this.rowsLength) {
      ...
      fillersToAdd =
        footerInsertStartIdx + footerToAdd.length - this.rowsLength;
    }
  ...
  private setupForNewSection(): void {
    ...
    this.internalRows = this.block.slice(
      this.blockRowsStartIdx,
      this.blockRowsEndIdx + 1
    );
  }
  ...
  private setupInternalBlock(): void {
    const endBlockLength = Math.max(
      this.rowsLength,
      this.getMinimumHeaderLength(),
      this.getMinimumFooterLength()
    );

    const header =
      this.internalHeader.length <= endBlockLength
        ? this.internalHeader +
          this.getSectionsFiller(endBlockLength - this.internalHeader.length)
        : this.internalHeader.slice(0, endBlockLength);

    const footer =
      this.internalFooter.length <= endBlockLength
        ? this.internalFooter +
          this.getSectionsFiller(endBlockLength - this.internalFooter.length)
        : this.internalFooter.slice(0, endBlockLength);

    const rows = this.internalRows.map((row) =>
      row.length < endBlockLength
        ? row + this.getRowFiller(endBlockLength - row.length)
        : row
    );
  ...
  private getMinimumSectionLength(section: string): number {
    const nonSectionFillerIdx = section.indexOfDifferent(
      this.tab.sectionFiller,
      -1,
      -1
    );
    const minimumSectionLenth =
      nonSectionFillerIdx > -1
        ? nonSectionFillerIdx + this.tab.rowsSpacing + 1
        : 0;

    return minimumSectionLenth;
  }

  private writeInstructionsToRows(
    instructions: TabBlockWriteInstruction[]
  ): void {
    const maxNoteLength = instructions.reduce(
      (maxNoteLength: number, instruction) => {
    ...
    this.internalRows.forEach((row, idx) => {
      const instructionToWrite = instructions.find(
        (instruction) => instruction.chord === idx + 1
      );
      if (instructionToWrite) {
        const rowFillerLength = maxNoteLength - instructionToWrite.note.length;
        this.internalRows[idx] =
          row + instructionToWrite.note + this.getRowFiller(rowFillerLength);
      } else {
  ...
  private getInvalidChordsToWrite(
    instructions: TabBlockWriteInstruction[]
  ): number[] {
  ...
  private getChordsWithMultipleInstructions(
    instructions: TabBlockWriteInstruction[]
  ): number[] {
    const chord2InstructionCountMap = instructions.reduce(
      (store: Record<number, number>, instruction) => {
        store[instruction.chord]
          ? store[instruction.chord]++
          : (store[instruction.chord] = 1);
        return store;
      },
      {}
    );

    const chordsWithMultipleInstructions = Object.keys(
      chord2InstructionCountMap
    )
  ...
  private getInvalidChordsToWriteDescription(chords: number[]): string {
    ...
    return chords.length === 1
      ...
      : `As cordas indicadas < ${chords.join(
          ', '
        )} > estão fora da faixa disponível, ${availableChordsDesc}`;
  }

  private getChordsWithMultipleInstructionsDescription(
    chords: number[]
  ): string {
    if (chords.length === 1) {
