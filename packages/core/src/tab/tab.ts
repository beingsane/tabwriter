...
export class Tab {
  ...
  public set sectionSymbol(value: string) {
    if (value.length !== 1)
      throw new InvalidArgumentError(
        'sectionSymbol must be a single character string'
      );
  ...
  constructor({
    ...
    if (rowsFiller !== undefined && rowsFiller.length !== 1)
      throw new InvalidArgumentError(
        'rowsFiller must be a single character string'
      );
    ...
    if (sectionFiller !== undefined && sectionFiller.length !== 1)
      throw new InvalidArgumentError(
        'sectionFiller must be a single character string'
      );

    this.currentRowsQuantity = rowsQuantity
      ? rowsQuantity
      : Tab.DEFAULT_ROWS_QUANTITY;
    this.currentRowsFiller = rowsFiller ? rowsFiller : Tab.DEFAULT_ROWS_FILLER;
    this.currentRowsSpacing = rowsSpacing
      ? rowsSpacing
      : Tab.DEFAULT_ROWS_SPACING;
    this.currentSectionSymbol = sectionSymbol
      ? sectionSymbol
      : Tab.DEFAULT_SECTION_SYMBOL;
    this.currentSectionFiller = sectionFiller
      ? sectionFiller
      : Tab.DEFAULT_SECTION_FILLER;
