...
export class InstructionFactory {
  ...
  private static extractChordAndNote(
    instructionSource: string
  ): ChordAndNoteExtractionResult {
    const chordAndNoteRegexMatchResult = InstructionFactory.CHORD_AND_NOTE_REGEX_EXTRACTION.exec(
      instructionSource
    );
  ...
  public static getInstruction(
    parsedInstructionResult: ParserResult
  ): Instruction {
    switch (parsedInstructionResult.method?.toUpperCase()) {
      ...
      case 'SEC':
      case 'SECTION':
        return InstructionFactory.getSectionInstruction(
          parsedInstructionResult
        );
      case 'S':
      case 'SPACE':
        return InstructionFactory.getSetSpacingInstruction(
          parsedInstructionResult
        );
      default:
        return InstructionFactory.getDefaultInstruction(
          parsedInstructionResult
        );
    }
  }

  private static getDefaultInstruction(
    parsedInstructionResult: ParserResult
  ): Instruction {
    const { chord, note } = InstructionFactory.extractChordAndNote(
      parsedInstructionResult.value
    );
  ...
  private static getMergeInstruction(
    parsedInstructionResult: ParserResult
  ): Instruction {
    if (
      !parsedInstructionResult.targets ||
      parsedInstructionResult.targets.length === 0
    )
      return new InvalidInstruction(
        'Nenhuma instrução indicada para aplicação da instrução merge'
      );
    ...
    if (unmergeableTargets.length > 0) {
      const unmergeableTargetsDesc = `< ${unmergeableTargets
        .map((target) => target.value)
        .join(' >, < ')} >`;
      return new InvalidInstruction(
        `As seguintes instruções são inválidas para merge: ${unmergeableTargetsDesc}`
      );
    }
  ...
  private static getNoteInstruction(
    parsedInstructionResult: ParserResult
  ): Instruction {
    if (
      !parsedInstructionResult.args ||
      parsedInstructionResult.args.length === 0
    )
      return new InvalidInstruction(
        'Nenhuma nota foi indicado para aplicação da instrução note'
      );

    if (parsedInstructionResult.args.length > 1)
      return new InvalidInstruction(
        'Apenas um argumento é utilizado para aplicação da instrução note'
      );

    return new NoteInstruction(parsedInstructionResult.args[0].toString());
  }

  private static getRepeatInstruction(
    parsedInstructionResult: ParserResult
  ): Instruction {
    if (
      !parsedInstructionResult.args ||
      ...
      return new InvalidInstruction(
        'A quantidade de repetições não foi indicada para aplicação da instrução repeat'
      );

    if (parsedInstructionResult.args.length > 1)
      return new InvalidInstruction(
        'Apenas um argumento é utilizado para aplicação da instrução repeat'
      );
    ...
    if (
      ...
      return new InvalidInstruction(
        'Nenhuma instrução indicada para aplicação da instrução repeat'
      );
  ...
  private static getSetSpacingInstruction(
    parsedInstructionResult: ParserResult
  ): Instruction {
    if (
      !parsedInstructionResult.args ||
      ...
      return new InvalidInstruction(
        'O novo espaçamento não foi indicado para aplicação da instrução space'
      );

    if (parsedInstructionResult.args.length > 1)
      return new InvalidInstruction(
        'Apenas um argumento é utilizado para aplicação da instrução space'
      );
  ...
  private static getSectionInstruction(
    parsedInstructionResult: ParserResult
  ): Instruction {
    if (
      !parsedInstructionResult.args ||
      ...
      return new InvalidInstruction(
        'O nome da seção não foi indicado para aplicação da instrução section'
      );

    if (parsedInstructionResult.args.length > 1)
      return new InvalidInstruction(
        'Apenas um argumento é utilizado para aplicação da instrução section'
      );
