...
import { TabBlockWriteResult } from '../tab/tabBlockWriteResult';
...
describe(`[${RepeatInstruction.name}]`, () => {
  ...
  it('should repeatedly write the instructions to repeat on write', () => {
    ...
    const instructionsToRepeat = [
      ...
      new DefaultInstruction(1, '2'),
    ];
    const instruction = new RepeatInstruction(
      repetitions,
      instructionsToRepeat
    );
    ...
    instructionsToRepeat.forEach(
      (instruction) =>
        (instruction.writeOnTab = jest
          .fn()
          .mockReturnValue(new TabBlockWriteResult(true)))
    );
  ...
  it('should write errors only once if the write of a instruction returns in error', () => {
    ...
    instructionsToRepeat.forEach((instruction, idx) => {
      instruction.writeOnTab = jest
        ...
        .mockReturnValue(
          idx > 0
            ? new TabBlockWriteResult(true)
            : new TabBlockWriteResult(false, errorDescription)
        );
    });

    const writeResult = instruction.writeOnTab(tab);
    const errorDescriptionMatchs = writeResult.description?.match(
      errorDescriptionMatcher
    );
